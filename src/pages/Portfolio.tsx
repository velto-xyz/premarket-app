...
export default function Portfolio() {
  ...
  const { data: activity = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const storage = new StorageLayer();
      const result = await storage.getUserTrades(address, 20);
  ...
  const { data: internalBalance = 0 } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const market of allMarkets) {
        if (market.perpEngineAddress) {
          const balance = await storage.contracts.getUserBalance(address, market.perpEngineAddress);
            ...
            {activity.length === 0 ? (
              ...
              <div className="space-y-2">
                {activity.map((trade) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${trade.type === "open"
